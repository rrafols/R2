...
- The next-train card and the list always look for the next trains regardless of the calendar: after the last train of the day the following service days are searched (up to 7), each with its own timetable type, and rows that run on a later date are tagged *demà* / weekday + date (with *festiu* when that day uses the holiday timetable). Live data only attaches to the current service day.
- Selecting on one side the station already chosen on the other swaps the trip instead of showing X → X.
...
- The map is centred on the selected origin station. If the browser grants geolocation and the device is in Catalonia, your position is shown as a magenta dot (the view is not moved).
